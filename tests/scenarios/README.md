...
模块测试只证明函数行为；**场景任务包**证明 Agent 能在真实用户需求下完成「理解需求 → 修改正确文件 → 运行验证 → 处理失败 → 解释结果 → 保留可审计轨迹」的完整闭环。每个阶段至少绑定一个真实场景，状态与安全改动必须通过对应 L2 场景。
...
里程碑不能只凭 L0/L1 完成；状态与安全改动至少通过对应 L2 场景。
